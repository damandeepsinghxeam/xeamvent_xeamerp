    ...
    <?php }else{ ?>
      ...
      <div class="three-icon-box">
          ...
          @php
            ...
            if(@$attendanceArray['late']) {
              $href = url('attendances/request-change');
              $anchorAttr = 'target="_blank"';
            }

            $mapUrl = url('attendances/view-map').'?id='.$user->id.'&date='.$cDayTime;
          @endphp  
          <a {!! $anchorAttr !!} href="{!! $href !!}" class="fc-day-grid-event fc-h-event fc-event fc-start fc-end fc-draggable calender-day-check-in @if(@$attendanceArray['late']){{'late'}}@endif">
