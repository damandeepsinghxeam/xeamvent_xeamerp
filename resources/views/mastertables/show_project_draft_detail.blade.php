  ...
  <section class="content">
    <div class="row">
      <div class="col-sm-12">
		...
		@$gst_no = $projects['project_approval']['gst_registration_number'];
		@$tan = $projects['project_approval']['tan_number'];
		
		@$h_o_loc = $projects['project_approval']['head_office_location'];
		@$h_o_add = $projects['project_approval']['head_office_address'];
		@$dep_name =  $projects['project_approval']['departments']->name;
		@$proj_owner = $projects['project_approval']['project_owner_name'];
		@$cont_name = $projects['project_approval']['contact_person'];
		@$off_add = $projects['project_approval']['corres_office_address'];
		...
		@$agreemnt_remark = $projects['project_approval']['agreement_remarks'];

		@$proj_start_date = $projects['project_approval']['starting_of_project_date'];
		@$tenure_years = $projects['project_approval']['contract_period_year'];
		@$tenure_months = $projects['project_approval']['contract_period_month'];
		...
		@$tds_deduct = $projects['project_approval']['tds_deduction'];

		if(@$projects['project_approval']['bg_security'] ==0){
			$security_required = "No";
		}else{
			$security_required = "Yes";
		}

		@$security_type = $projects['project_approval']['bg_or_security'];
		@$security_amount = $projects['project_approval']['bg_or_security_amount'];

		@$security_status = $projects['project_approval']['bg_or_security_submission'];
		@$security_valadity = $projects['project_approval']['bg_or_security_to_client'];
		@$project_location_state = $projects['project_approval']['states'];


		
		@$project_location_city = $projects['project_approval']['cities'];
		@$resource_category = $projects['project_approval']['resources_category'];
		...
		@$salary_date = $projects['project_approval']['salary_payment_date'];
		@$salary_mode = $projects['project_approval']['salary_mode'];

		if(@$projects['project_approval']['esi_applicable'] ==0){
			$esi_apply = "No";
		}else{
			$esi_apply = "Yes";
		}

		if(@$projects['project_approval']['epf_applicable'] ==0){
			$epf_apply = "No";
		}else{
			$epf_apply = "Yes";
		}

		if(@$projects['project_approval']['gpa_applicable'] ==0){
			$gpa_apply = "No";
		}else{
			$gpa_apply = "Yes";
		}

		if(@$projects['project_approval']['wc_applicable'] ==0){
			$wc_apply = "No";
		}else{
			$wc_apply = "Yes";
		}		
		...
		if(@$projects['project_approval']['online_application_charges'] ==0){
		...
		if(@$projects['project_approval']['tic_or_Medical_insurance'] ==0){
			$insurance = "No";
		}else{
			$insurance = "Yes";
		}
		...
		@$form = $projects['bg_data']['form_type'];
		@$amount = $projects['bg_data']['amount'];
		@$in_favour = $projects['bg_data']['in_favour_of'];
		@$date_required = $projects['bg_data']['required_date'];
		@$bg_valadity_date = $projects['bg_data']['validity_date'];
		@$margin_percentage = $projects['bg_data']['margin_value'];
		@$bg_charges_percentage = $projects['bg_data']['bg_charges_value'];
		@$gst_percentage = $projects['bg_data']['gst_value'];
		@$margin = $projects['bg_data']['margin'];
		@$bg_charges = $projects['bg_data']['bg_charges'];
		@$gst = $projects['bg_data']['gst'];
		@$total = $projects['bg_data']['total_charges'];
		
		
		//it form 
		@$sms = $projects['it_data']['sms'];
		@$sms_no = $projects['it_data']['number_of_sms'];
		@$email = $projects['it_data']['email'];
		@$email_no = $projects['it_data']['number_of_email'];
		...
		@$premium_amount = $projects['ins_data']['total_premium_amount'];
		@$in_favor = $projects['ins_data']['in_favour_of'];
		@$date_required = $projects['ins_data']['required_date'];
		@$peers_no = $projects['ins_data']['number_of_peers_or_lives'];
		@$peers_note = $projects['ins_data']['extra_note'];
		@$expense_borne_by = $projects['ins_data']['expense_borne'];
		...
		@$tenure_years = $projects['ins_data']['project_tenure_year'];
		@$tenure_months = $projects['ins_data']['project_tenure_month'];
		@$invoice_no = $projects['ins_data']['invoice_number'];

		@$person = $projects['ins_data']['person'];
			
		@$quote_amount = $projects['ins_data']['amount'];
								...
								<table class="table table-bordered table-striped viewTilTable">
									...
									<tbody>
										...
										<tr>
											<td>TAN No.</td>
											<td>
												@if(isset($tan) AND (is_array($tan)))
												@php
												$i=1;
												@endphp
												@foreach($tan as $tan_no)
										...
										<tr>
											...
											<td>Years: {{$tenure_years}} &nbsp; &nbsp; Months: {{$tenure_months}}</td>
										</tr>
										<tr>
											<td>Extendable</td>
											<td>{{$extendable}}</td>
										...
										<tr>
											...
											<td>
												@if(isset($project_location_state) )
												<span>State:</span>
											...
											@if(isset($project_location_city) )
												<span>City: </span>
										...
										<tr>
											...
											<td> @if(isset($resource_category) AND (is_array($resource_category)))
												@foreach($resource_category as $res_category)
					                           		<span>{{$res_category}}, </span>
												@endforeach
											@endif</td>
										</tr>
									</tbody>
								...
								<table class="table table-bordered table-striped viewTilTable">
									...
									<tbody>
										<tr>
											...
											<td><span class="viewFileSpan label label-default">@if(!empty(@$projects['project_approval']['agreement_with_customer']))<a target="_blank" class="" href="{{config('constants.uploadPaths.projectDocument')}}{{@$projects['project_approval']['agreement_with_customer']}}">view</a>@endif</span></td>
										</tr>
										<tr>
											<td>Proposed Agreement with Employee</td>
											<td><span class="viewFileSpan label label-default">@if(!empty(@$projects['project_approval']['proposed_agreement']))<a target="_blank" class="" href="{{config('constants.uploadPaths.projectDocument')}}{{@$projects['project_approval']['proposed_agreement']}}">view</a>@endif</span></td>
										</tr>
										<tr>
											<td>Format of Offer Letter</td>
											<td><span class="viewFileSpan label label-default">@if(!empty(@$projects['project_approval']['offer_letter_format']))<a target="_blank" class="" href="{{config('constants.uploadPaths.projectDocument')}}{{@$projects['project_approval']['offer_letter_format']}}">view</a>@endif</span></td>
										</tr>
									</tbody>
								</table>
